...
from micrograd.optimizers import Adam, SGD
...
def test_adam_first_step_manual():
    ...
    # At t=1, bias correction makes m_hat equal g and v_hat equal g**2.
    # Therefore the update is approximately lr * sign(g).
    expected_w = 2.0 - 0.1 * 4.0 / (abs(4.0) + 1e-8)
    expected_b = -1.0 - 0.1 * (-2.0) / (abs(-2.0) + 1e-8)

    assert abs(w.data - expected_w) < 1e-12
    assert abs(b.data - expected_b) < 1e-12
    assert optimizer.step_count == 1
    assert abs(optimizer.m[id(w)] - 0.4) < 1e-12
    assert abs(optimizer.v[id(w)] - 0.016) < 1e-12


def test_adam_multi_step_loop_vs_pytorch():
    # Micrograd setup
    w = Value(1.5)
    b = Value(0.5)
    ...
    w_pt = torch.tensor(1.5, dtype=torch.float64, requires_grad=True)
    b_pt = torch.tensor(0.5, dtype=torch.float64, requires_grad=True)
    opt_pt = torch.optim.Adam([w_pt, b_pt], lr=0.01)
    ...
    for _ in range(10):
        ...
        assert abs(w.data - w_pt.item()) < 1e-9
        assert abs(b.data - b_pt.item()) < 1e-9
